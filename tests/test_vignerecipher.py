...
def test_vigenere_encode_vignere_decode():
    ...
    res_encoded = vc.vignere_encode(row, key, keystream, message)
    
    assert res_encoded == "qknevwskjjmsz"
    assert vc.vignere_decode(row, key, keystream, res_encoded) == message
